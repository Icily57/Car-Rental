...
const AllsVehSpecs = () => {
  ...
  const [rentalRate, setRentalRate] = useState<number>(0); // Rental rate state
  const [vehiclesSpecs, setVehiclesSpecs] = useState<VehicleSpecs[]>([]);  // const [showModal, setShowModal] = useState(false);
  ...
  const handleAddVehicle = async () => {
    ...
    const vehicledata: VehicleFormValues = {
      vehicleSpec_id: id, rental_rate: rentalRate}
    try {      
  ...
  <div> 
    ...
    <dialog id="my_modal_5" className={`modal modal-bottom sm:modal-middle ${isModalOpen ? 'modal-open' : ''}`}>
      <div className="modal-box">
        ...
        <div className="modal-action">
          <button className="btn btn-outline btn-info" onClick={handleAddVehicle} >
            Add Vehicle
          </button>
